...
formula against the published ORGAN signal estimate, a Monte Carlo propagation
of all input uncertainties ($m_3$, $\chi_{\mathrm{top}}$, $c_\nu$) to the
target-frequency distribution, and two concrete test protocols: (A)~a
...
\label{eq:seesaw}
\end{equation}
where $c_\nu$ is an unknown dimensionless coefficient. After EWSB
($\langle H \rangle = v_{\mathrm{EW}}/\sqrt{2}$; the factor of 2 compensates
the two factors of $1/\sqrt{2}$) this reduces to the Majorana mass, for the
heaviest neutrino,
\begin{equation}
...
\sqrt{\frac{t}{\Delta f}},
\end{equation}
the \emph{preregistered} search window is derived by Monte Carlo propagation
of all inputs (Appendix~\ref{app:repro}): $m_3 \sim \mathcal{N}(50.3,\,
0.25)$~meV, $\chi_{\mathrm{top}}^{1/4} \sim \mathcal{N}(75.5,\,0.7\%)$~MeV,
and $c_\nu$ treated as a parameter rather than an error.
\begin{itemize}
...
\item \textbf{Benchmark scenario ($c_\nu$ log-uniform in $[1/2,\,2]$).} The
95\% interval widens to $6.7$--$24.9$~GHz; a discovery program over this band
carries the corresponding trials factor, which we state honestly.
...
point dark-matter target: $m_\theta = 53.2~\mu$eV, a microwave line at
