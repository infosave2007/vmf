...
\label{eq:seesaw}
\end{equation}
where $c_\nu$ is an unknown dimensionless coefficient. Since $c_\nu$ already
absorbs the UV normalization, we \emph{define} it through the physical
relation~\eqref{eq:m3} below; the overall prefactor of
Eq.~\eqref{eq:seesaw} --- including the displayed factor of 2, which
compensates the two factors of $1/\sqrt{2}$ from
$\langle H \rangle = v_{\mathrm{EW}}/\sqrt{2}$ --- is then conventional and
carries no independent physical meaning. After EWSB the operator reduces to
the Majorana mass, for the heaviest neutrino,
\begin{equation}
...
\sqrt{\frac{t}{\Delta f}},
\end{equation}
the \emph{preregistered} search window combines a Monte Carlo propagation of
the statistical inputs (Appendix~\ref{app:repro}),
$m_3 \sim \mathcal{N}(50.3,\,0.25)$~meV,
$\chi_{\mathrm{top}}^{1/4} \sim \mathcal{N}(75.5,\,0.7\%)$~MeV, with explicit
conditional scenarios for the unknown coefficient $c_\nu$. We treat $c_\nu$ as
a parameter, not a random error: no prior on it is motivated, so the
$c_\nu$-dependent ranges below are \emph{sensitivity scenarios}, not
statistical credible intervals.
\begin{itemize}
\item \textbf{Benchmark window ($c_\nu = 1$ fixed).} The target-frequency
distribution has median $12.86$~GHz and a 68\% interval
$[12.67,\,13.05]$~GHz (relative width $1.5\%$, dominated by
$\chi_{\mathrm{top}}$). The preregistered window is the $3\sigma$ envelope
$12.86\,\text{GHz} \times (1 \pm 4.4\%) = [12.29,\,13.43]$~GHz, resolved in
bins of width $\Delta f_a$.
\item \textbf{Sensitivity scenario ($c_\nu$ varied over $[1/2,\,2]$).} Via
Eq.~\eqref{eq:scaling} the conditional band spans $6.4$--$25.7$~GHz; a
discovery program over this band carries the corresponding trials factor,
which we state honestly.
...
\end{itemize}
\paragraph{Signal power across the band.} Along the curve~\eqref{eq:scaling},
\emph{at fixed coherent fraction}, the signal powers scale simply:
$P_{\mathrm{sig}}^{\mathrm{stack}} \propto g^2/m_\theta^2 \propto c_\nu^{0}$
for the dielectric stack (exactly invariant under $c_\nu$ rescaling) and
$P_{\mathrm{sig}}^{\mathrm{cavity}} \propto g^2/m_\theta \propto c_\nu^{-1}$
for the cavity, so the integration times of
...
\propto c_\nu^{7/6}$, which multiplies both powers accordingly.
The $\pm 0.5\%$ window of the v1 draft covered only $m_3$ and
$\chi_{\mathrm{top}}$ and is superseded. Because the benchmark target is a
point rather than a band, the look-elsewhere penalty inside the preregistered
window is negligible; no trials factor beyond it is applied to the benchmark
test.
