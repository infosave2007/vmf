...
def calculate_axion_parameters(m_omega: float) -> tuple[float, float]:
    ...
    m_pi = 0.13957         # GeV
    f_pi = 0.0924          # GeV
    r_h0 = 1.37e23         # km (present Hubble horizon)
    
    # Genesis instanton scale: r_c = 1.13 * (859 / M_omega)
    r_c = 1.13 * (859.0 / m_omega)
    n_e = math.log(r_h0 / r_c)
    
    # Axion decay constant: f_a = M_planck / N_e^4
    f_a = m_planck / (n_e ** 4)
...
def main():
    ...
    print(f"Genesis e-folds N_e   : {math.log(1.37e23 / (1.13 * (859.0/m_omega_center))):.2f}")
