...
def run_mass_derivation_check():
    ...
    # In the Higgsless limit (current quark masses m_q -> 0):
    # M_p -> M_Omega = 859.0 MeV
    # M_pi -> 0 MeV (strictly massless Goldstone limit)
    # Note: nucleon mass is generated nonperturbatively by chiral symmetry breaking
    # and the trace anomaly (gluon confinement) of QCD, not by the electroweak Higgs mechanism.
    
    # Under VMF, the nonperturbative mass scale is anchored by M_Omega.
    # The physical pion mass arises entirely from the explicit chiral symmetry breaking by the quark mass (generated by the Higgs mechanism).
