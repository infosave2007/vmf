#!/usr/bin/env python3
"""
NVG dense matter: beta-equilibrated core EOS with saturating vector repulsion.

This is a follow-up to nvg_eos_beta_checked.py.
The only structural change is the isoscalar vector sector:
instead of a constant quadratic term 0.5 * Cw * n_B^2, the repulsion
is made density-dependent above saturation density to test whether the
superluminal sound-speed problem is mainly a constant-vector artifact.
"""
...
def vector_factor(n_b, alpha_v, nu_v):
    """Switch on vector saturation only above n0 and keep g(1)=1 exactly."""
    x = n_b / n_0
    if x <= 1.0:
        return 1.0
...
def calibrate_c_omega0(k1, k2, c_s, c_rho):
    ...
    if not np.isfinite(c_omega0) or c_omega0 <= 0.0:
        return None, None
    return c_omega0, state_n0


def build_eos(k1, k2, c_s, c_rho, c_omega0, alpha_v, nu_v, nn=180):
    ...
    for i, n_b in enumerate(narr):
        ...
        if state is None:
            continue
        eps[i] = state["eps_no_vec"] + vector_energy_density(n_b, c_omega0, alpha_v, nu_v)
    ...
    return narr_g, eps_g, pressure_g, mdirac_g, yprot_g, cs2_g
...
def solve_tov(eps_of_p, p_c):
    ...
    sol = solve_ivp(rhs, [r0, 40.0], [m0, p_c], events=stop, max_step=0.15, rtol=1.0e-6, atol=1.0e-10)
...
def run_model(k1, k2, c_s, c_rho, alpha_v, nu_v, with_tov=False):
    c_omega0, state_n0 = calibrate_c_omega0(k1, k2, c_s, c_rho)
    if c_omega0 is None:
        return None
    eos = build_eos(k1, k2, c_s, c_rho, c_omega0, alpha_v, nu_v)
    if eos is None:
        return None
...
def main():
    parser = argparse.ArgumentParser(description="NVG beta-equilibrated core EOS with saturating vector repulsion")
    parser.add_argument(
        "--with-tov",
        action="store_true",
        help="also run an expensive TOV scan for the screened models",
    )
    args = parser.parse_args()

    print("=" * 88)
    print("NVG BETA-SATURATED: DENSITY-DEPENDENT VECTOR REPULSION TEST")
    if args.with_tov:
        print("fixed lattice input -> beta-equilibrated EOS -> saturating vector sector -> optional TOV")
    else:
        print("fixed lattice input -> beta-equilibrated EOS -> saturating vector sector screening")
    print("NOTE: this remains a core-only prototype; no crust matching yet")
    print("=" * 88)
    ...
    tests = [
        (0.20, 0.60, 900.0, 600.0, 1.0, 2.0),
        (0.20, 0.60, 900.0, 600.0, 2.0, 2.0),
        (0.20, 0.60, 900.0, 600.0, 4.0, 2.0),
        (0.25, 0.80, 900.0, 600.0, 1.0, 2.0),
        (0.25, 0.80, 900.0, 600.0, 2.0, 2.0),
        (0.25, 0.80, 900.0, 600.0, 4.0, 2.0),
        (0.30, 0.80, 1200.0, 600.0, 1.0, 2.0),
        (0.30, 0.80, 1200.0, 600.0, 2.0, 2.0),
        (0.30, 0.80, 1200.0, 600.0, 4.0, 2.0),
        (0.30, 1.00, 1400.0, 900.0, 1.0, 2.0),
        (0.30, 1.00, 1400.0, 900.0, 2.0, 2.0),
        (0.30, 1.00, 1400.0, 900.0, 4.0, 2.0),
    ]

    results = []
    for k1, k2, c_s, c_rho, alpha_v, nu_v in tests:
        result = run_model(k1, k2, c_s, c_rho, alpha_v, nu_v, with_tov=False)
        if result is not None:
            results.append(result)

    if not results:
        print("No numerically viable saturated-vector EOS curves were produced.")
        return 1

    print(f"Scanned models: {len(tests)}")
    print(f"Numerically viable saturated-vector EOS curves: {len(results)}")
    print()
    print(
        f"{'k1':>4} {'k2':>4} {'alpha':>5} {'nu':>3} {'M_D/M':>7} {'Yp(n0)':>7} "
    ...
    for result in results:
        core_ok, sat_ok = screening_flags(result)
        if core_ok:
            core_passes.append(result)
        print(
            f"{result['k1']:4.2f} {result['k2']:4.2f} {result['alpha_v']:5.1f} {result['nu_v']:3.0f} "
    ...
    if args.with_tov and core_passes:
        print("Running TOV sanity scan for screened models...")
        tov_results = []
        for result in core_passes[:4]:
            rerun = run_model(
                result['k1'], result['k2'], result['Cs'], result['Crho'], result['alpha_v'], result['nu_v'], with_tov=True
            )
            if rerun is not None:
                tov_results.append(rerun)
        if tov_results:
            print()
            print(f"{'k1':>4} {'k2':>4} {'alpha':>5} {'Mmax':>7} {'R1.4':>6}")
            print("-" * 34)
            for result in tov_results:
                ...
                print(f"{result['k1']:4.2f} {result['k2']:4.2f} {result['alpha_v']:5.1f} {mmax_str:>7} {r14_str:>6}")
        else:
            print("TOV scan did not return valid stellar branches for the screened models.")
    elif not args.with_tov:
        print("TOV scan: skipped by default; use --with-tov if the core screening succeeds.")

    if core_passes:
        best = sorted(core_passes, key=lambda item: (item['cs2_max'], abs(item['p_n0'])))[0]
        print()
        print("Best saturated-vector core-screening point:")
        print(f"  k1 = {best['k1']:.2f}, k2 = {best['k2']:.2f}")
        print(f"  alpha_v = {best['alpha_v']:.1f}, nu_v = {best['nu_v']:.1f}")
        ...
        best = sorted(results, key=lambda item: item['cs2_max'])[0]
        print()
        print("No point passed the saturated-vector core screening.")
        print(f"Best reduction still had c_s^2,max = {best['cs2_max']:.3f} at alpha_v = {best['alpha_v']:.1f}.")
