...
def coupled_system(y, a, beta_melt, w_W):
    """
...
def derive_w0_wa(m_omega: float = 859.0) -> tuple[float, float]:
    ...
    # 1. Vacuum Condensate tracking: The W-field has a natural baseline 
    #    equation of state w_W = -0.9, which gives it a quintessence behavior.
    ...
    # Baseline W-field equation of state
    w_W = -0.9
    ...
    sol = odeint(coupled_system, y0, a_vals, args=(beta_melt, w_W))
...
def main():
    ...
    print("  Baseline W-field EOS w_W         : -0.90")
    print("-" * 74)
