#!/usr/bin/env python3
"""
NVG Dark Photon Observables: conditional model scans for a density-activated
dark photon.  The calculations are sensitivity/forecast surfaces and do not
constitute an independent confirmation without sourced likelihoods.
...
def main():
    L = []
    L.append("=" * 80)
    L.append("       CONDITIONAL DARK-PHOTON OBSERVABLE SCANS")
    L.append("Generated by: verification/nvg_dark_photon_observables.py")
    L.append("=" * 80)
    ...
    for t in t_vals:
        ...
        role = "omega>0.12 (assumption check)" if omega > 0.12 else "omega<=0.12 (assumption check)"
    ...
    for ev in eps_vac_vals:
        ...
        status = "tau<1s criterion" if tau < 1.0 else "tau>=1s criterion"
        L.append(f"  {ev:16.1e} | {width:18.4e} | {tau:18.4e} | {status:<10}")
        
    L.append("\n  Interpretation: the relic and lifetime rows are model-dependent calculations")
    L.append("      conditional on the stated thermal history and decay channel; they are not")
    L.append("      an independent BBN exclusion/permission without a sourced likelihood.")
    ...
    L.append("\n  Interpretation: this is a conditional centrality-scaling forecast. No detector")
    L.append("      response, background model, or traceable HADES/CBM likelihood is included.")
    ...
    L.append("\n  Interpretation: h_0 is a conditional model forecast for the supplied field/spin/")
    L.append("      distance inputs. Detectability is not assessed because no detector noise curve")
    L.append("      or search threshold is supplied.")
    ...
    for m in mass_values:
        ...
        status = "below Raffelt reference" if L_A < 3.0e52 else "above Raffelt reference"
        L.append(f"  {m:22.3f} GeV | {drop:13.1f}% | {L_A:22.3e} | {status:<10}")
        
    L.append("\n  Interpretation: luminosities are a sensitivity scan against the stated Raffelt")
    L.append("      reference value. Without a sourced SN1987A likelihood and validated transport")
    L.append("      model, the rows do not establish a mass-drop bound.")
    L.append("=" * 80)
