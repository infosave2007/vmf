...
def build_eos(k1, k2, c_s, c_rho, c_omega0, alpha_v, nu_v):
    ...
    for n_b in narr:
        ...
        if state is None:
            continue
        eps_val = state["eps_no_vec"] + vector_energy_density(n_b, c_omega0, alpha_v, nu_v)
        eps.append(eps_val)
        valid_n.append(n_b)
...
def solve_tov(eps_of_p, p_c):
    ...
    sol = solve_ivp(rhs, [r0, 40.0], [0.0, p_c], events=stop, max_step=0.2, rtol=1.0e-5, atol=1.0e-8)
...
def main():
    ...
    for alpha_v in [0.0, 0.05, 0.1, 0.15, 0.2, 0.3]:
        c_omega0 = calibrate_c_omega0(k1, k2, c_s, c_rho)
        narr, eps, pressure = build_eos(k1, k2, c_s, c_rho, c_omega0, alpha_v, nu_v)
        good = pressure > 0
        ...
        densities_s = np.array(densities)[m_sorted_idx]
        
        nc_at_145 = -1.0
        if masses_s[0] < 1.45 < masses_s[-1]:
            nc_at_145 = np.interp(1.45, masses_s, densities_s)
            
        print(f"alpha_v = {alpha_v:.3f} | has_negative={has_negative_pressure} | nc(1.45) = {nc_at_145:.3f} n0 | Max Mass = {masses_s[-1]:.3f} M_sun")
