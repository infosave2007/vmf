...
| 19 | Majorana neutrino mass sum: $\sum m_\nu \approx 0.117$ eV (scale estimate; GUT-scale prior) | DESI DR2 excludes this at 95% within $\Lambda$CDM ($< 0.064$ eV) but allows it in the $w_0 w_a$ cosmology ($< 0.16$ eV) that NVG itself predicts — the two claims are co-testable ([neutrino_mass.py](verification/nvg_neutrino_mass.py)). Note: row 52 gives an alternative mapping with $\sum m_\nu = 59$ meV — the two are mutually exclusive and must be reconciled | ⚙️ Scale estimate (viable only jointly with $w_0 w_a$) |
...
| 35 | $S_8$ structure growth | Honest accounting: the NVG dynamical dark energy slightly increases growth ($+1.4\%$), giving $S_8 \approx 0.843$ — $4.0\sigma$ from the lensing value (vs $3.3\sigma$ in $\Lambda$CDM); the proposed de Sitter core mechanism is $\sim 45$ orders of magnitude short of the required $7.8\%$ suppression ([s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Open problem (potential falsifier) |
...
| 42 | $S_8$ structure growth (see row 35 — same calculation) | The de Sitter core suppression cannot contribute at Mpc scales (DM core volume fraction $\sim 10^{-47}$); honest $S_8 \approx 0.843$ ([nvg_s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Open problem (duplicate of row 35) |
