...
The finite instanton size $r_c$ stretched by $N_e \approx 53$ e-folds (calibrated to the local $H_0$) spans the present Hubble horizon by construction; the falsifiable content is the resulting deterministic infrared cutoff that suppresses CMB multipoles $\ell=2,3$. Tested against Planck 2018 TT ([nvg_cmb_lowl_refit.py](verification/nvg_cmb_lowl_refit.py)): the cutoff improves the low-$\ell$ fit by $\Delta\chi^2 \approx +0.9$ with the best-fit scale within $\approx 1.2\times$ of the predicted $k_c = 1/R_{H0}$ — a real but mild ($\sim 1\sigma$) and shape-dependent effect ($-1.0$ to $+1.9$ across cutoff steepness and likelihood form), also consistent with cosmic variance; it does not shift the CMB-inferred $H_0$.
...
| 2 | $M_{\max} = 2.05\,M_\odot$ (computed; the earlier 2.25 was a table-edge artifact — see [nvg_ns_parameter_scan.py](verification/nvg_ns_parameter_scan.py)) | PSR J0740+6620: $2.08 \pm 0.07\,M_\odot$ ($-0.4\sigma$); any confirmed NS above $\sim 2.2\,M_\odot$ excludes the model | ⚠️ Compatible, edge-falsifiable |
| 3 | $R_{1.4} = 12.55$ km (computed from the canonical EOS; the earlier 12.0 was hardcoded) | NICER J0030: $12.2 \pm 0.5$ km ($+0.7\sigma$); J0437: $11.4 \pm 1.0$ km ($+1.2\sigma$); $R_{1.4} < 12.0$ km would exclude the model | ⚠️ Compatible, edge-falsifiable |
...
| 5 | NS Cooling Dichotomy via direct-Urca threshold at $1.45 M_\odot$ | Cas A (slow) vs Vela (fast); NOTE: the threshold position is **calibrated** — $\alpha_v$ in [nvg_direct_urca.py](verification/nvg_direct_urca.py) is tuned to place it at 1.45 (and differs from the canonical EOS $\alpha_v$) | ⚠️ Calibrated |
| 6 | Tidal deformability: $\Lambda_{1.4} = 519$, binary $\tilde\Lambda \approx 610$ (TOV + Hinderer $y$-integration, [nvg_tidal_deformability.py](verification/nvg_tidal_deformability.py); the earlier 177 belonged to a parameterization falsified by J0740) | GW170817: $\tilde\Lambda = 300\,[70, 720]$ — inside the 90% CI at $+0.8\sigma$; $\tilde\Lambda < \sim 400$ in a future event excludes the model | ⚠️ Compatible, edge-falsifiable |
...
| 13 | Effective vacuum dielectric constant: $\varepsilon_{\rm eff} \approx 0.135\,\varepsilon_0$ in NS cores (from $e^{-2\alpha_v f_{\rm melt}}$ with hand-set melting parameters $\kappa$; $\kappa_1 = 0.21$ here vs $0.25$ in the EOS — unreconciled) | Would amplify magnetar seed fields by $1/\sqrt{\varepsilon_{\rm eff}} \approx 2.7\times$ | ⚠️ Scale estimate ($\kappa$-dependent) |
| 14 | Relic Dark Matter: $\Omega_{\rm DM} = 0.268$ is an **observational input** — the script inverts it to infer the VMF self-coupling $\lambda_v$ | The checkable content is that the inferred $\lambda_v$ maps to the $f_0(1370)$–$f_0(1500)$ scalar range ([nvg_relic_dark_matter.py](verification/nvg_relic_dark_matter.py)) | ⚠️ Calibrated (inverted) |
| 15 | NS core speed of sound: $c_{s}^2 \to 1/3$ in the quark phase **by construction** ($c_{s,q}^2 = 1/3$ is an input of the CSS ansatz, not a derived result) | Compatible with joint NICER+LIGO posterior limits ([speed_of_sound_curve.py](verification/nvg_speed_of_sound_curve.py)) | ⚠️ By construction |
...
| 17 | Joint NS multi-messenger inference: reduced $\chi_\nu^2 \approx 0.7$ with the canonical (computed) predictions | All pulls $< 1\sigma$, **conditional on** the nuclear-calibrated EOS shape parameters and the scan-selected transition — not a zero-parameter fit ([nvg_joint_ns_inference.py](verification/nvg_joint_ns_inference.py)) | ⚠️ Compatible (conditional) |
