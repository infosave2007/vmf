...
**Singularity resolution mechanism:** At density $\rho \to \rho_c$, the strong energy condition is violated ($\varepsilon + 3P < 0$), halting collapse and replacing the singularity with a de Sitter vacuum. This is the same mechanism that produces the cosmological bounce, applied at stellar scales. Topologically, this de Sitter core acts as a **Ricci flow fixed point (Einstein manifold soliton)**. Just as in Perelman's proof of the Poincaré conjecture where such points maximize the reduced volume and halt further topological collapse ($\kappa$-non-collapsing theorem), the NVG core's convergence to a pure de Sitter geometry at $\rho_c$ provides a rigorous geometric guarantee of stability, replacing the standard SEC-violation argument with a structural topological limit.
...
| 26 | NICER PSR J0437-4715 Radius | Canonical EOS gives $R \approx 13.1$ km at $1.418\,M_\odot$ vs observed $11.36 \pm 0.8$ km: $+2.2\sigma$, outside the 95% CI — the tightest tension of the canonical model ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Tightest tension ($+2.2\sigma$) |
...
| 28 | Baryon asymmetry | **No-Go Theorem and Dark Neutron.** A rigorous proof (`nogo_baryogenesis_bounded_T.tex`) that standard spontaneous baryogenesis is impossible without proton decay in a bounded-temperature cosmology ($T_{\max} \approx 432$ MeV). The only viable solution (Class I) is **B-L Cogenesis via a dark neutron $\chi$** ($m_\chi \approx 938.34$ MeV). Protected from proton decay by exact symmetry, predicts a falsifiable dark matter fraction and branching $Br(n \to \chi \gamma) \sim 10^{-8}$ ([nvg_adm_bl_cogenesis.py](verification/nvg_adm_bl_cogenesis.py)). | 📐 Rigorous (No-Go Theorem / Cogenesis) |
...
| 37 | Neutron star core g-modes | Fundamental $l=2$ composition g-mode period is $T_g \approx 66$ ms, within 50-150 ms ([ns_g_modes.py](verification/nvg_ns_g_modes.py)) | ✅ Confirmed |
| 38 | SN1987A Core Cooling | Density-activated dark photon maintains in-medium mass drop $\le 20\%$ at $n_0$, successfully keeping anomalous core energy loss $L_{\rm loss} < 3 \times 10^{52}$ erg/s within the Raffelt limit ([nvg_dark_photon_observables.py](verification/nvg_dark_photon_observables.py)) | ✅ Limit Respected |
...
| 50 | Antimatter as $\theta \to -\theta$ | C-conjugation = Goldstone phase reversal. Strict baryogenesis mechanism is provided by Spontaneous Baryogenesis via topological winding $\dot{\theta}$ ([nvg_baryogenesis_bsm_closure.py](verification/nvg_baryogenesis_bsm_closure.py)). Annihilation = vortex reconnection with $\tau_{ann} = \hbar/(k_B T) = \tau_{collapse}$. Anti-universes = cycles with $Q = -1$ ([antimatter_topology.py](verification/nvg_antimatter_topology.py)) | 📐 Rigorous (Spontaneous Baryogenesis) |
...
| 54 | Critical Horizon Mass | $M_{\rm crit} \approx 0.99\,M_\odot$. Objects lighter than this (e.g., primordial) cannot form event horizons. Note: this is a fundamental gravitational limit, completely distinct from the astrophysical stellar "mass gap" ($2-5\,M_\odot$) driven by supernova mechanics. | 🔭 Forward prediction |

**5. Universal Entropy Remnant & Analytical Deficit:**
By scanning the horizon roots of the Hayward metric as $M \to \infty$, we discover a profound analytical limit: the inner horizon $r_{\rm in}$ asymptotes to the universal vacuum length scale $l = \sqrt{3c^2 / (8\pi G \rho_c)} \approx 1.128$ km. Consequently, the core of *any* macroscopic black hole in the NVG model encapsulates a strictly conserved, indestructible information remnant:
$$ S_{\rm in}(M \to \infty) \to \frac{k_B c^3 (4\pi l^2)}{4G\hbar} \approx 2.2 \times 10^{76} \text{ bits} $$
Furthermore, an asymptotic expansion of the outer horizon $r_{\rm out} \approx r_{\rm Sch} - l^2 c^2 / (2GM)$ reveals that the total entropy deficit relative to a classical Schwarzschild black hole is exactly equal to the inner core entropy: $\Delta S = S_{\rm Sch} - (S_{\rm out} + S_{\rm in}) = S_{\rm in}$. The core acts as an exact informational quantum of $10^{76}$ bits (roughly $10^{-46}$ of the observable universe's entropy) sealed within the singularity-free vacuum.
