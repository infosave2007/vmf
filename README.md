...
| 10 | Surface gravitational redshift: $z_{\rm surf} = 0.223$ for $1.4\,M_\odot$ NS (from the computed $R_{1.4} = 12.49$ km) | Direct measurements are currently absent; forward prediction testable by STROBE-X/eXTP ([nvg_ns_redshift.py](verification/nvg_ns_redshift.py)) | 🔭 Forward prediction |
...
| 29 | NICER PSR J0437-4715 Radius | Canonical EOS gives $R = 12.49$ km at $1.418\,M_\odot$ vs observed $11.36 \pm 0.8$ km: $+1.4\sigma$, outside the 68% CI but inside 95% — the tightest tension of the canonical model ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Tightest tension ($+1.4\sigma$) |
...
| 50 | Heisenberg uncertainty | $\Delta x \cdot \Delta p \geq \hbar/2$ is rigorously derived as the classical Fourier limit (Cauchy-Schwarz inequality) for the Madelung hydrodynamics of the vacuum condensate, where $\hbar$ serves purely as the dimensionful scale coupling phase gradient to momentum ($p = \hbar\nabla\theta$). | 📐 Rigorous (Classical Hydrodynamics) |
