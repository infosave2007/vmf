...
**Status legend:** ✅ compatible with data · 📏 interval prediction · 🔭 forward prediction (no measurement yet) · ⚙️ calibration / consistency check · ⚪ null test · 📐 mathematical result (not an experimental confirmation) · ❓ conjectural · ❌ open problem / retracted · ⏳ awaiting experiment. Falsifiable rows state the measurement that would exclude the model. Statuses are per-row; no global look-elsewhere correction across the table is claimed.
...
| 18 | Scalar Glueball mass: $M_{\rm glueball} = 2 M_\Omega \approx 1.72$ GeV | Lightest scalar glueball ($0^{++}$) from the trace-anomaly excitation vs Lattice QCD $1.7 \pm 0.1$ GeV | ✅ Compatible (within lattice uncertainty) |
| 19 | Majorana neutrino mass sum, GUT-prior mapping: $\sum m_\nu \approx 0.117$ eV | **Retired** in favor of the $\theta$-seesaw sector (row 52): this mapping is excluded at 95% in $\Lambda$CDM by DESI DR2 ($< 0.064$ eV) and survives only jointly with $w_0 w_a$, while the seesaw mapping passes all current bounds unconditionally ([neutrino_mass.py](verification/nvg_neutrino_mass.py)) | ❌ Retired (superseded by row 52) |
...
| 24 | JWST SMBH Mass Spectrum (z = 6–15) | A seed on the $N=10$ rung of the PBH ladder grows into the JWST objects sub-Eddington while Pop III seeds fail; conditional on that rung being occupied — the repo's own PBH abundance (peaked at $N=-21$, $\sigma_N = 1.3$) assigns it essentially zero weight, so a two-population abundance model is required | ⚙️ Conditional (abundance model needed) |
...
| 29 | NICER PSR J0437-4715 Radius | Canonical EOS gives $R = 12.55$ km at $1.418\,M_\odot$ vs observed $11.36 \pm 0.8$ km: $+1.5\sigma$, outside the 68% CI but inside 95% — the tightest tension of the canonical model ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Tightest tension ($+1.5\sigma$) |
...
| 31 | Baryon asymmetry $\eta_B \approx 6 \times 10^{-10}$ | Dimensional ansatz $\pi\sqrt{T_b/M_{\rm Pl}}$: the 1/2 exponent and prefactor are selected post hoc, and QCD-scale baryogenesis additionally requires B-violation beyond the SM (electroweak sphalerons are frozen at $T \ll 130$ GeV) ([baryon_asymmetry.py](verification/nvg_baryon_asymmetry.py)) | ⚙️ Post-hoc ansatz (mechanism open) |
| 32 | Post-merger peak frequency $f_{\rm peak} \approx 2.42$ kHz | From the canonical TOV radius $R_{1.6} = 12.65$ km; no post-merger signal has been observed yet — testable by LIGO O5 / Einstein Telescope ([postmerger_fpeak.py](verification/nvg_postmerger_fpeak.py)) | 🔭 Forward prediction |
...
| 35 | $S_8$ structure growth | Honest accounting: the NVG dynamical dark energy slightly increases growth ($+1.4\%$), giving $S_8 \approx 0.843$ — $4.0\sigma$ from the lensing value (vs $3.3\sigma$ in $\Lambda$CDM); the proposed de Sitter core mechanism is $\sim 45$ orders short of the required $7.8\%$; moreover, in the CMB-anchored frame the mass-melting sector that fits DESI $w_0 w_a$ *raises* $S_8$ to $\approx 0.90$ ($7\sigma$) — weak lensing directly constrains that sector ([s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Open problem (potential falsifier) |
| 36 | NANOGrav SGWB from PBH-binary mergers | The $f^{2/3}$ shape is generic for binary backgrounds; the amplitude rests on a binary fraction $f_{\rm bin} = 0.0041$ quoted to three digits and on heavy-rung PBH abundance that the repo's own distribution (row 38: peak $N=-21$, $\sigma_N = 1.3$) sets to essentially zero — internally inconsistent as published ([nanograv_background.py](verification/nvg_nanograv_background.py)) | ⚙️ Calibrated; abundance inconsistency to resolve |
...
| 41 | Hubble constant (see row 9 — same calculation) | Cycle-77 horizons bound $H_0 \in [54.3, 108.5]$ km/s/Mpc with mid-cycle value $\approx 76.8$; the point value $72.8$ is set by the local $H_0$, and the IR cutoff leaves the CMB-inferred $H_0$ unchanged ([nvg_hubble_tension.py](verification/nvg_hubble_tension.py), [nvg_cmb_lowl_refit.py](verification/nvg_cmb_lowl_refit.py)) | 📏 Interval prediction (duplicate of row 9) |
| 42 | $S_8$ structure growth (see row 35 — same calculation) | The de Sitter core suppression cannot contribute at Mpc scales (DM core volume fraction $\sim 10^{-47}$); honest $S_8 \approx 0.843$, and the mass-melting configuration that fits DESI $w_0 w_a$ raises it further to $\approx 0.90$ ([nvg_s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Open problem (duplicate of row 35) |
...
| 45 | Strong CP Problem Solution | $\bar{\theta}_{\rm QCD} = 0$ automatically: global minimum of $V(W_0, \theta)$ is at $\theta = 0$ due to vacuum condensate structure. No Peccei-Quinn mechanism needed, given the identification $\bar\theta_{\rm QCD} \equiv \theta$ (a physical postulate) ([strong_cp_solution.py](verification/nvg_strong_cp_solution.py)) | 📐 Semi-rigorous derivation |
...
| 47 | Double-slit interference from vacuum hydrodynamics | $|\psi|^2$ pattern reproduced by Huygens-Fresnel integral over vacuum phase $\theta$ (Madelung representation), $r_{\rm Pearson} = 1.000$ ([double_slit_madelung.py](verification/nvg_double_slit_madelung.py)) | 📐 Reproduces standard QM (Madelung) |
...
| 49 | Bell-CHSH correlations in the condensate | The correlation form $E = -\cos(a-b)$ is postulated (preprint Limitations: conjectural); by Bell's theorem a local derivation is impossible — a shared phase read out locally is a hidden variable giving $S \le 2$, so any derivation from the action must contain an explicitly nonlocal or contextual element. The NVG-specific content is the temperature dependence $S(T > T_c) \to 0$ (row 56) ([bell_inequality.py](verification/nvg_bell_inequality.py)) | ❓ Conjectural (correlation form postulated) |
| 50 | Heisenberg uncertainty — theorem, not postulate | $\Delta x \cdot \Delta p \geq \hbar/2$ = Cauchy-Schwarz inequality for $\nabla\log\mathcal{W}$ and $\nabla\theta$ of the vacuum condensate. The inequality is rigorous mathematics; $\hbar$ enters through the identification $p = \hbar\nabla\theta$ — the postulate is relocated, not eliminated ([heisenberg_proof.py](verification/nvg_heisenberg_proof.py)) | 📐 Theorem (given $p = \hbar\nabla\theta$) |
...
| 53 | Quantum gravity without quantization | $T_H = \hbar c^3/(8\pi G M k_B) = \hbar/(k_B \tau_\theta)$ — Hawking temperature = θ-field thermalization at horizon (exact match). $S_{BH} = A/(4l_{Pl}^2) = N_\theta$ — entropy = θ-mode counting. Bounce at $\rho_c/\rho_{Pl} \sim 10^{-77}$ — fully semiclassical ([quantum_gravity.py](verification/nvg_quantum_gravity.py)) | ❓ Conjectural (heuristic identification; preprint Limitations) |
| 54 | Fine structure constant from vacuum polarization | One-loop estimate $1/\alpha(M_Z) = 126.6$ vs $127.95$ measured ($\sim 1\%$ off). Note: the script uses $W_0 = 432.2$ MeV, which the audit identified as the bounce temperature $T_b$, not the vacuum value ($859$ MeV) — needs recomputation ([fine_structure.py](verification/nvg_fine_structure.py)) | ⚙️ 1-loop estimate (needs $W_0$ fix) |
| 55 | Antimatter as $\theta \to -\theta$ | C-conjugation = Goldstone phase inversion. $\eta_B$ from topological choice $Q = +1$ at bounce. Annihilation = vortex reconnection with $\tau_{ann} = \hbar/(k_B T) = \tau_{collapse}$. Anti-universes = cycles with $Q = -1$ ([antimatter_topology.py](verification/nvg_antimatter_topology.py)) | ❓ Conjectural (interpretational mapping) |
...
| 57 | Homochirality from QCD topology | Biological homochirality (L-amino acids, D-sugars) is fixed by cosmological bounce topological charge $Q=+1$, yielding cell-scale Goldstone coherence $\xi_\theta \approx 7.6\,\mu\text{m}$ at 300 K ([nvg_dna_chirality.py](verification/nvg_dna_chirality.py)) | ❓ Speculative hypothesis (scale estimates only) |
...
| Joint Multi-Messenger Inference | Reduced $\chi^2_\nu \approx 1.0$ for the joint fit of NICER, LIGO and cooling data (calibrated rows excluded). | Achieved with the QCD anchor plus nuclear-calibrated EOS shape parameters — no astrophysical fitting. |
...
| Dark Energy (DESI) | Cyclic evolution: $w_0 = -0.876$, $w_a = -0.667$ | Derived from first principles via the coupled Einstein-Boltzmann system (mass-melting Dark Matter): qualitative alignment with the DESI 2024 phantom-crossing quadrant, though exhibiting a $\sim 4.8\sigma$ quantitative tension; note the same melting configuration raises $S_8$ to $\approx 0.90$ in the CMB-anchored frame, so weak lensing and DESI $w_0 w_a$ cannot both be satisfied by this sector as formulated ([nvg_dark_energy_w0wa.py](verification/nvg_dark_energy_w0wa.py) / [nvg_dark_energy_desi.py](verification/nvg_dark_energy_desi.py)) |
...
| **Meson Mass Shift** | Observable integrated dielectron peak at $\approx 712$ MeV (instantaneous $M_\rho^* \approx 621$ MeV at $2n_0$; the integrated peak is what HADES measures) | HADES simulation script ready; request submitted to HADES / CBM Collaboration (FAIR) |
...
| **NS Gravitational Redshift** | $z_{\rm surf}(1.4 M_\odot) \approx 0.221$ (canonical $R_{1.4} = 12.55$ km) | STROBE-X / eXTP (future X-ray observatories) |
| **Post-merger $f_{\rm peak}$** | $f_{\rm peak} \approx 2420$ Hz (canonical $R_{1.6} = 12.65$ km) | LIGO O5 / Einstein Telescope (future detectors) |
...
- **DESI 2024 Dark Energy $w(z)$:** The predicted cyclic cosmology trajectory ($w_0 = -0.876, w_a = -0.667$) is derived strictly from first principles via the coupled Einstein-Boltzmann equations representing the mass-melting Dark Matter. This dynamic evolution natively calculates the W-field Lagrangian, successfully reproducing the qualitative phantom crossing ($w_a < 0$) observed by DESI, though it exhibits a $\sim 4.8\sigma$ tension with the exact center of the joint confidence ellipse. The same melting configuration raises $S_8$ to $\approx 0.90$ in the CMB-anchored frame ($7\sigma$ from lensing), so this sector cannot simultaneously satisfy DESI and weak lensing as formulated.
- **GW170817 Tidal Deformability:** The canonical EOS yields binary tidal deformability $\tilde{\Lambda} \approx 610$, inside the LIGO 90% CI ($[70, 720]$, $+0.8\sigma$ from the center); a future event with $\tilde{\Lambda} < 400$ would exclude the model.
...
- **NICER PSR J0437-4715 Radius:** The canonical radius $R = 12.55$ km is $+1.5\sigma$ above the 2024 NICER measurement ($11.36 \pm 0.8$ km at $1.418 M_\odot$) — inside 95% but the tightest tension of the model; a confirmed $R < 12.0$ km would stress the canonical EOS (`verification/nvg_nicer_j0437_check.py`).
- **NANOGrav 15yr SGWB:** A PBH-binary merger background reproduces the $f^{2/3}$ shape (generic for binaries); the amplitude depends on the assumed binary fraction ($f_{\rm bin} = 0.0041$) and on heavy-rung PBH abundance, which the repo's own PBH distribution currently sets to zero — an internal inconsistency to resolve. The bounce itself, per the derived $(\alpha, \beta/H)$, radiates at $18$–$42\,\mu$Hz, not in the PTA band (`verification/nvg_nanograv_background.py`, `verification/nvg_recondensation_dynamics.py`).
