...
**Status legend:** ✅ compatible with data · 📏 interval prediction · 🔭 forward prediction (no measurement yet) · ⚙️ calibration / consistency check · ⚪ null test · 📐 mathematical result (not an experimental confirmation) · ❓ conjectural · ❌ open problem / retracted · ⏳ awaiting experiment. Falsifiable rows state the measurement that would exclude the model. Statuses are per-row; the global statistics are computed in [nvg_global_significance.py](verification/nvg_global_significance.py): of the 57 rows, 7 are independent quantitative tests, jointly giving $\chi^2_{\rm red} = 0.68$ ($p = 0.69$ — compatible, not suspiciously good), with the largest pull ($+1.5\sigma$, J0437) below the $\sim 2.3\sigma$ trials-corrected expectation. Provenance note: $T_c = 157$ MeV, used across the scripts, is *adopted* from lattice QCD (HotQCD: $156.5 \pm 1.5$ MeV) — an input identification, not an NVG prediction, and it is deliberately excluded from the test count.
...
| 3 | $R_{1.4} = 12.55$ km (canonical EOS) | NICER J0030: $12.2 \pm 0.5$ km ($+0.7\sigma$); J0437: $11.36 \pm 0.8$ km ($+1.5\sigma$, see row 29). Exclusion criterion: $R_{1.4} < 12.0$ km | ✅ Compatible |
...
| 22 | Topological Axion Mass | **Superseded by the theta-sector audit** ([nvg_theta_sector_audit.py](verification/nvg_theta_sector_audit.py)): this row's ($8.38\,\mu$eV, $f_a = 1.54\times 10^{12}$ GeV) conflicts with row 52's adopted axion ($53\,\mu$eV, $1.07\times 10^{11}$ GeV) by $\times 6.3$ in mass — the same duplication pattern as the retired second neutrino sector. Merged into the row-52 sector; note the condensate phase itself is the $\eta'$ (958 MeV), so the light axion is a separate extension field ([axion_mass.py](verification/nvg_axion_mass.py)) | ❌ Retired (merged into row 52) |
...
| 31 | Baryon asymmetry | **No mechanism within the current field content** (computed): the bounce caps the temperature at $T_b = 432$ MeV in *every* cycle, so sphalerons/leptogenesis are unavailable in principle; a BSM $(qqql)/\Lambda^2$ operator needs $\Lambda \approx 550$ TeV but is excluded by proton decay by $\sim 43$ orders, and both in-model gates fail; inheritance through cycles is excluded by 13 orders ([nvg_baryogenesis_bsm_closure.py](verification/nvg_baryogenesis_bsm_closure.py), [nvg_etab_inheritance.py](verification/nvg_etab_inheritance.py)). The $\pi\sqrt{T_b/M_{\rm Pl}}$ ansatz is withdrawn as a claim. A viable **extension** is constructed in [nvg_adm_bl_cogenesis.py](verification/nvg_adm_bl_cogenesis.py): B$-$L cogenesis through a neutron portal with a dark fermion $\chi$ — proton-safe by exact B$-$L, nucleon-safe by the kinematic corridor $937.90 < m_\chi < 938.78$ MeV; one calibrated scale ($\Lambda = 556$ TeV) and a falsifiable floor ${\rm Br}(n \to \chi\gamma) \sim 4\times 10^{-6}$ ($\times 27$ below current bounds); predicts an 18.6% dark-neutron DM subcomponent, but does **not** derive $\Omega_{\rm DM}/\Omega_b = 5.36$ (integer multiples excluded at $>5\sigma$) | ⚙️ Extension constructed (calibrated; lab-testable floor) |
...
| 36 | NANOGrav SGWB | **Retired.** The heavy-PBH population calibrated to the JWST requirement (the only abundance passing the CMB-accretion bound) gives a strain $\sim 1{,}700\times$ below NANOGrav, and the bounce itself radiates at $\mu$Hz — NVG has no mechanism for the NANOGrav signal, which is attributed to ordinary SMBH binaries ([nvg_pbh_two_population.py](verification/nvg_pbh_two_population.py)) | ❌ Retired (no NVG mechanism) |
