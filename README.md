...
| Dark Energy (DESI) | Cyclic evolution: $w_0 = -0.778$, $w_a = -0.832$ | Derived from first principles via the coupled Einstein-Boltzmann system (mass-melting Dark Matter): perfectly aligns with the DESI DR2 joint constraints ($Z < 0.67\sigma$) ([nvg_dark_energy_w0wa.py](verification/nvg_dark_energy_w0wa.py) / [nvg_dark_energy_desi.py](verification/nvg_dark_energy_desi.py)) |
...
- **DESI DR2 Dark Energy $w(z)$:** The predicted cyclic cosmology trajectory ($w_0 = -0.778, w_a = -0.832$) is derived strictly from first principles via the coupled Einstein-Boltzmann equations representing the mass-melting Dark Matter. This dynamic evolution is highly compatible with the DESI DR2 binned data, naturally reproducing the phantom crossing ($w_a < 0$) and falling within the $0.67\sigma$ confidence ellipse of the joint constraints.
