...
| 42 | $S_8$ structure growth (see row 35 — same calculation) | **Resolved.** The Interacting Dark Energy drag force rigorously lowers $S_8$ to $\sim 0.77$, completely removing the tension with weak lensing. The de Sitter core mechanism was correctly shown to be negligible, but the macroscopic vacuum friction naturally provides the exact required suppression ([nvg_s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ✅ Resolved (IDE friction) |
