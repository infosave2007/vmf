...
The dynamics of the vacuum condensate amplitude are derived from the polar representation of the complex order parameter $\Phi(x) = \frac{1}{\sqrt{2}} \mathcal{W}(x) e^{i \theta(x)}$ under a $U(1)_B$ gauge covariant derivative $D_\mu = \partial_\mu - i q A_\mu$. Here, $A_\mu = \frac{g_\omega}{m_\omega^2} J_\mu$ is identified as the effective baryonic vector meson ($\omega$-meson) field coupled to the baryon 4-current $J_\mu = n_B u_\mu$, where the baryon density $n_B \equiv J_\mu u^\mu$ is a strict Lorentz scalar.

$$
\begin{aligned}
\text{Kinetic term: } & (D_\mu \Phi)^\dagger (D^\mu \Phi) = \frac{1}{2} \partial_\mu \mathcal{W} \partial^\mu \mathcal{W} + \frac{1}{2} \mathcal{W}^2 (\partial_\mu \theta - q A_\mu)(\partial^\mu \theta - q A^\mu) \\
\text{Equation of motion: } & \square \mathcal{W} + \left[ \lambda \mathcal{W}^2 - (\mu^2 + g_\mu g^\mu) \right] \mathcal{W} = 0 \quad \text{where } g_\mu \equiv \partial_\mu \theta - q A_\mu = \mu_\theta u_\mu - \gamma j_{B,\mu} \\
\text{Energy-Momentum: } & T^{\mathcal{W}}_{\mu\nu} = \partial_\mu \mathcal{W} \partial_\nu \mathcal{W} + \mathcal{W}^2 g_\mu g_\nu - g_{\mu\nu} \left[ \frac{1}{2} \partial_\alpha \mathcal{W} \partial^\alpha \mathcal{W} + \frac{1}{2} \mathcal{W}^2 g_\alpha g^\alpha - V(\mathcal{W}) \right]
\end{aligned}
$$

* **Lorentz Covariance & Baryon Current Identification:** The generalized momentum $g_\mu$ is identified with the normalized baryon 4-current $j_B^\mu \equiv J^\mu / n_0$ as $g_\mu = \mu_\theta u_\mu - \gamma j_{B,\mu}$, where $\gamma \equiv \frac{q g_\omega n_0}{m_\omega^2} \approx 955.5$ MeV. Since $u_\mu u^\mu = 1$, the contraction $g_\mu g^\mu = (\mu_\theta - \gamma \frac{n_B}{n_0})^2$ is a manifest Lorentz scalar, ensuring covariance.
* **QCD Calibration & Bounds:** In deep vacuum ($n_B = 0$, $A_\mu = 0$), the expectation value is anchored to the $\Omega$-baryon mass $\mathcal{W}_0 = M_{\Omega,0} = \sqrt{(\mu^2 - \mu_\theta^2)/\lambda} = 859$ MeV. The potential parameters $\mu$ and $\lambda$ are calibrated via the pion-nucleon ($\sigma_{\pi N} \approx 44$ MeV) and strange ($\sigma_{sN} \approx 30$ MeV) QCD sigma-terms, fixing $\lambda \approx 1.05$ (corresponding to scalar mass $m_{\mathcal{W}} = \sqrt{2\lambda}\mathcal{W}_0 \approx 1245$ MeV, matching the $f_0(1370)$ meson) and constraining $q = \frac{g_\omega M_{\Omega,0}}{2 f_\pi} \approx 47.01$ with zero free parameters.
* **SEC Violation & Bounce:** In FLRW spacetime, the energy-momentum tensor reduces to $\rho_{\mathcal{W}} = \frac{1}{2} \dot{\mathcal{W}}^2 + \frac{1}{2} \mathcal{W}^2 g_0^2 + V(\mathcal{W})$ and $P_{\mathcal{W}} = \frac{1}{2} \dot{\mathcal{W}}^2 - \frac{1}{2} \mathcal{W}^2 g_0^2 - V(\mathcal{W})$. As density exceeds $n_B \ge 2.5 n_0$, the field melts ($\mathcal{W} \to 0$), driving the potential to its symmetric barrier $V(0) = \frac{1}{4}\lambda\mathcal{W}_0^4 > 0$. This violates the Strong Energy Condition ($\rho_{\mathcal{W}} + 3 P_{\mathcal{W}} \to -2 V(0) < 0$). In this regime, the coupling dynamics yield $\rho_{\mathcal{W}} \approx -\rho_{\text{matter}}^2 / \rho_c$, modifying the Einstein equations to reproduce the Friedmann bounce:
