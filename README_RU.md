...
| 3 | $R_{1.4} = 12.55$ км (канонический EOS) | NICER J0030: $12.2 \pm 0.5$ км ($+0.7\sigma$); J0437: $11.36 \pm 0.8$ км ($+1.5\sigma$, см. строку 29). Критерий исключения: $R_{1.4} < 12.0$ км | ✅ Совместимо |
...
| 7 | $\rho$-мезон в среде: мгновенная масса $-20\%$ при $2n_0$ ($M_\rho^* \approx 621$ МэВ, связь $W$-поля, $\kappa_1 = 0.21$). **Наблюдаемый** сдвиг пика диэлектронов, усреднённый по истории файербола, $\approx -7\%$ ($\approx 712$ МэВ) — именно это меряет HADES | Спектры диэлектронов Au+Au/Ag+Ag от HADES ([nvg_hades_dielectron_sim.py](verification/nvg_hades_dielectron_sim.py)); пик у $\approx 712$ МэВ, отличимый от чистого уширения ($\approx 775$), — это тест | ⏳ Ожидает HADES |
...
| 32 | Пост-мержерная пиковая частота $f_{\rm peak} \approx 2.42$ кГц | Из канонического TOV-радиуса $R_{1.6} = 12.65$ км; пост-мержерный сигнал ещё не наблюдался — проверяемо LIGO O5 / Einstein Telescope ([postmerger_fpeak.py](verification/nvg_postmerger_fpeak.py)) | 🔭 Форвардное предсказание |
...
| **Сдвиг масс мезонов** | Наблюдаемый интегрированный пик диэлектронов $\approx 712$ МэВ (мгновенная $M_\rho^* \approx 621$ МэВ при $2n_0$; HADES меряет интегрированный пик) | Скрипт симуляции HADES готов; направлен запрос в коллаборацию HADES/CBM (FAIR) |
...
| **Красный сдвиг НЗ** | $z_{\rm surf}(1.4 M_\odot) \approx 0.221$ (канонический $R_{1.4} = 12.55$ км) | STROBE-X / eXTP (будущие рентгеновские обсерватории) |
| **Пост-мержер $f_{\rm peak}$** | $f_{\rm peak} \approx 2420$ Гц (канонический $R_{1.6} = 12.65$ км) | LIGO O5 / Einstein Telescope (будущие детекторы) |
