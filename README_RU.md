...
| 42 | Решение проблемы сильного CP-нарушения | $\bar{\theta}_{\rm QCD} = 0$ автоматически: глобальный минимум потенциала $V(W_0, \theta)$ находится при $\theta = 0$ из-за структуры вакуумного конденсата. Механизм Пеккеи-Куинн не нужен — при условии отождествления $\bar\theta_{\rm QCD} \equiv \theta$ (физический постулат) ([strong_cp_solution.py](verification/nvg_strong_cp_solution.py)) | 📐 Полустрогий вывод |
| 43 | Стрела времени из топологии | Энтропийный ток $s^\mu = s \cdot u^\mu$, $u^\mu \propto \partial^\mu \theta$ — монотонный рост энтропии следует из $Q = (1/2\pi)\oint d\theta = 1 > 0$. H-теорема для направления времени; количественный энтропийный бюджет остаётся открытым ([arrow_of_time.py](verification/nvg_arrow_of_time.py)) | 📐 Теорема (качественно) |
| 44 | Двухщелевая интерференция из вакуумной гидродинамики | Паттерн $|\psi|^2$ воспроизводится интегралом Гюйгенса-Френеля по фазе $\theta$ вакуумного конденсата (представление Маделунга), $r_{\rm Pearson} = 1.000$ ([double_slit_madelung.py](verification/nvg_double_slit_madelung.py)) | 📐 Воспроизводит стандартную КМ (Маделунг) |
