...
| 35 | Пик доли ПЧД как ТМ | Дискретная лестница масс следует из теории (шаг ступени $\times 2$ по исправленному закону Толмена; сетка $4^N$ предшествует ему); положение пика обилия ($N=-21$) и ширина ($\sigma_N = 1.3$) размещены в наблюдательно разрешённом астероидном окне ([pbh_dark_matter.py](verification/nvg_pbh_dark_matter.py)) | ⚙️ Лестница предсказана; обилие калибровано |
...
1. **Эхо гравитационных волн:** Строгое предсказание $\Delta t_{\rm echo} \approx 0.022$ с (22.0 мс) с учётом спина для слияния ЧД массой 65 $M_\odot$ (безпараметрическое, проверяемое LIGO/Virgo).
