...
| 24 | Спектр масс JWST SMBH (z = 6–15) | Иерархия масс $4^N$ описывает ранние тяжелые зародыши, допуская субэддингтоновскую аккрецию против Pop III семян; условно на заселённость ступени $N=10$, которой текущее распределение ПЧД (пик $N=-21$, $\sigma_N = 1.3$) не даёт — нужна двухпопуляционная модель обилия | ⚙️ Условно (нужна модель обилия) |
...
| 36 | Стохастический ГВ-фон NANOGrav от слияний ПЧД | Форма $f^{2/3}$ универсальна для бинарных фонов; амплитуда опирается на долю двойных $f_{\rm bin} = 0.0041$ (три значащие цифры) и на обилие тяжёлых ступеней ПЧД, которое собственное распределение репозитория (строка 38: пик $N=-21$, $\sigma_N = 1.3$) обнуляет — внутреннее противоречие, требующее разрешения ([nanograv_background.py](verification/nvg_nanograv_background.py)) | ⚙️ Калибровка; противоречие в обилии |
...
| 51 | Постоянная тонкой структуры из вакуумной поляризации | Одноконтурная оценка $1/\alpha(M_Z) = 126.6$ против измеренных $127.95$ ($\sim 1\%$ расхождения). Замечание: скрипт использует $W_0 = 432.2$ МэВ — аудит установил, что это температура отскока $T_b$, а не вакуумное значение ($859$ МэВ); требуется пересчёт ([fine_structure.py](verification/nvg_fine_structure.py)) | ⚙️ Одноконтурная оценка (нужен пересчёт $W_0$) |
