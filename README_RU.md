...
Динамика амплитуды вакуумного конденсата выводится из полярного представления комплексного параметра порядка $\Phi(x) = \frac{1}{\sqrt{2}} \mathcal{W}(x) e^{i \theta(x)}$ под действием $U(1)$ ковариантной производной $D_\mu = \partial_\mu - i q A_\mu$:
...
\text{Уравнение движения: } & \square \mathcal{W} + \left[ \lambda \mathcal{W}^2 - (\mu^2 + g_\mu g^\mu) \right] \mathcal{W} = 0 \quad \text{где } g_\mu \equiv \partial_\mu \theta - q A_\mu \\
\text{Эффективный потенциал: } & V_{\text{eff}}(\mathcal{W}) = \frac{1}{2} (|\vec{g}|^2 - \mu^2) \mathcal{W}^2 + \frac{1}{4} \lambda \mathcal{W}^4 \quad \text{при } g_\mu g^\mu \approx -|\vec{g}|^2
\end{aligned}
$$

* **Симметричная фаза (Плавление):** Если напряженность внешнего поля ниже порога ($|\vec{g}|^2 \le \mu^2$), поле имеет ненулевое вакуумное ожидаемое значение $\mathcal{W}_0 = \sqrt{(\mu^2 - |\vec{g}|^2)/\lambda}$. При превышении порога ($|\vec{g}|^2 \ge \mu^2$) эффективная квадратичная масса меняет знак на положительный, переводя минимум потенциала в $\mathcal{W}_0 = 0$ (полное плавление конденсата).
...
| 26 | Бесхиггсовское отношение масс нуклона и пиона | Формальный вывод отношения масс бариона и пиона ($M_p \approx 941.4$ МэВ, $M_\pi \to 0$ в киральном пределе) с привязкой к $M_\Omega = 859$ МэВ без механизма Хиггса | ✅ Точное совпадение |
