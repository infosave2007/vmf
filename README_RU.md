...
| 46 | Корреляции Белла-CHSH в конденсате | Форма корреляции $E = -\cos(a-b)$ постулирована (Limitations препринта: гипотеза); по теореме Белла локальный вывод невозможен — общая фаза, считываемая локально, есть скрытая переменная с $S \le 2$, так что вывод из действия обязан содержать явно нелокальный или контекстуальный элемент. Специфика NVG — температурная зависимость $S(T > T_c) \to 0$ (строка 53) ([bell_inequality.py](verification/nvg_bell_inequality.py)) | ❓ Гипотеза (форма корреляции постулирована) |
