...
| 31 | Барионная асимметрия $\eta_B \approx 6 \times 10^{-10}$ | Размерный анзац $\pi\sqrt{T_b/M_{\rm Pl}}$: показатель 1/2 и префактор выбраны задним числом; кроме того, бариогенезис на масштабе КХД требует нарушения B за пределами СМ (электрослабые сфалероны заморожены при $T \ll 130$ ГэВ) ([baryon_asymmetry.py](verification/nvg_baryon_asymmetry.py)) | ⚙️ Пост-хок анзац (механизм открыт) |
...
| 46 | Корреляции Белла-CHSH в конденсате | Форма корреляции $E = -\cos(a-b)$ постулирована (Limitations препринта: гипотеза); по теореме Белла локальный вывод невозможен — общая фаза, считываемая локально, есть скрытая переменная с $S \le 2$, так что вывод из действия обязан содержать явно нелокальный или контекстуальный элемент. Контекстуальная конструкция продемонстрирована численно: динамика $\theta$ на конфигурационном пространстве воспроизводит $S = 2\sqrt{2}$ детерминированными траекториями, а сепарабельная пространственно-временная $\theta$ упирается в $S = 2$ ([nvg_bell_contextual.py](verification/nvg_bell_contextual.py)) — открытым остаётся вывод конфигурационной структуры из действия. Специфика NVG — температурная зависимость $S(T > T_c) \to 0$ (строка 53) | ❓ Вмещается, но не выводится (конструкция явная) |
...
| 52 | Антивещество как $\theta \to -\theta$ | C-сопряжение = обращение фазы Голдстоуна. $\eta_B$ из топологического выбора $Q = +1$ при отскоке. Аннигиляция = пересоединение вихрей с $\tau_{ann} = \hbar/(k_B T) = \tau_{collapse}$. Антивселенные = циклы с $Q = -1$ ([antimatter_topology.py](verification/nvg_antimatter_topology.py)) | ❓ Гипотеза (интерпретационное отображение) |
