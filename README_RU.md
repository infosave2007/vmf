...
| 31 | Барионная асимметрия | **Механизма в текущем наборе полей нет** (вычислено): отскок ограничивает температуру $T_b = 432$ МэВ во *всех* циклах — сфалероны/лептогенезис недоступны в принципе; BSM-оператор $(qqql)/\Lambda^2$ требует $\Lambda \approx 550$ ТэВ, но исключён распадом протона на $\sim 43$ порядка, оба внутримодельных «затвора» не работают; наследование через циклы исключено на 13 порядков ([nvg_baryogenesis_bsm_closure.py](verification/nvg_baryogenesis_bsm_closure.py), [nvg_etab_inheritance.py](verification/nvg_etab_inheritance.py)). Анзац $\pi\sqrt{T_b/M_{\rm Pl}}$ отозван как заявление. Жизнеспособное **расширение** построено в [nvg_adm_bl_cogenesis.py](verification/nvg_adm_bl_cogenesis.py): B$-$L-когенезис через нейтронный портал с тёмным фермионом $\chi$ — протон защищён точной B$-$L-симметрией, нуклоны — кинематическим коридором $937.90 < m_\chi < 938.78$ МэВ; одна калиброванная шкала ($\Lambda = 556$ ТэВ) и фальсифицируемый пол ${\rm Br}(n \to \chi\gamma) \sim 4\times 10^{-6}$ (в 27 раз ниже текущих границ); предсказывает 18.6% тёмно-нейтронную компоненту ТМ, но **не** выводит $\Omega_{\rm DM}/\Omega_b = 5.36$ (целые кратные исключены на $>5\sigma$) | ⚙️ Расширение построено (калибровка; проверяемо в лаборатории) |
...
| 35 | Рост структуры $S_8$ | Честный учёт: динамическая тёмная энергия NVG слегка усиливает рост ($+1.4\%$), давая $S_8 \approx 0.843$ — $4.0\sigma$ от лензинга (против $3.3\sigma$ в $\Lambda$CDM); предложенный механизм ядер де Ситтера короче требуемых $7.8\%$ на $\sim 45$ порядков; более того, в системе отсчёта с якорем на CMB сектор плавящейся ТМ, фитирующий DESI $w_0 w_a$, *повышает* $S_8$ до $\approx 0.90$ ($7\sigma$) — слабое линзирование напрямую ограничивает этот сектор ([s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Открытая проблема (потенциальный фальсификатор) |
...
| 52 | Антивещество как $\theta \to -\theta$ | C-сопряжение = обращение фазы Голдстоуна. Маршрут «$\eta_B$ из топологического выбора $Q = +1$» закрыт: аномалия КХД сохраняет B (SU(3) векторная), а электрослабые инстантоны при $T \le T_b$ заморожены на уровне $10^{-164}$ ([nvg_baryogenesis_bsm_closure.py](verification/nvg_baryogenesis_bsm_closure.py)). Аннигиляция = пересоединение вихрей с $\tau_{ann} = \hbar/(k_B T) = \tau_{collapse}$. Антивселенные = циклы с $Q = -1$ ([antimatter_topology.py](verification/nvg_antimatter_topology.py)) | ❓ Гипотеза (интерпретационное отображение) |
...
| 54 | Гомохиральность из топологии КХД | Биологическая гомохиральность (L-аминокислоты, D-сахара) фиксируется топологическим зарядом $Q = +1$ при космологическом отскоке, обеспечивая когерентность голдстоуновской фазы на масштабе клетки $\xi_\theta \approx 7.6$ мкм при $300$ К ([nvg_dna_chirality.py](verification/nvg_dna_chirality.py)) | ❓ Спекулятивная гипотеза (только оценки масштабов) |
...
- **Слабое линзирование $S_8$:** Открытая проблема. Честный учёт даёт $S_8 \approx 0.843$ ($4.0\sigma$ от лензинга): динамическая тёмная энергия NVG слегка усиливает рост, а механизм ядер ПЧД не может действовать на Мпк-масштабах (`verification/nvg_s8_tension_check.py`).
...
- **S8 (дубль пункта выше):** см. честный учёт — $S_8 \approx 0.843$, открытая проблема (`verification/nvg_s8_tension_check.py`).
