...
| 4 | Инстантон Генезиса $r_c \to$ $N_e = 53.08$ е-фолда (выводится из возраста/горизонта) | $R_{H0} = c/H_0 \approx 1.27 \times 10^{28}$ см (выводится из $t_0 \approx 13.8$ млрд лет) | ✅ Точное совпадение |
...
| 6 | Тидальная деформируемость: $\Lambda_{1.4} \approx 177$ (выведена из TOV + Hinderer $y$-интегрирования) | GW170817: $\Lambda_{1.4} = 190^{+390}_{-120}$ (попадание прямо в центр posterior) | ✅ Точное совпадение |
...
| 9 | Постоянная Хаббла: $H_0 = 72.8$ км/с/Мпк (выводится из $r_c \to N_e \to R_{H0}$) | Локальные измерения (SH0ES: $73.04 \pm 1.04$ км/с/Мпк) выводятся из горизонта поворота 77-го цикла Толмена ($R_{77} = r_c \cdot 2^{76}$) при текущей фазе расширения $\theta \approx 48^\circ\text{-}54^\circ$ ([hubble_tension.py](verification/nvg_hubble_tension.py)) | ✅ Разрешает натяжение |
...
| 22 | Топологическая масса аксиона: $m_a \approx 8.38 \times 10^{-6}$ эВ ($f_a \approx 1.54 \times 10^{12}$ ГэВ) | Находится в пределах поискового окна ADMX ($1.0 - 10.0\,\mu\text{эВ}$ / $10^{-6} - 10^{-5}$ эВ, [axion_mass.py](verification/nvg_axion_mass.py)) как топологическая оценка масштаба | ✅ Совместимо (Оценка масштаба) |
