...
**Ключевое преимущество над конкурентами:** В отличие от generic Бардина/Хейворда (свободный параметр $g$), fuzzball ($10^{500}$ вакуумов теории струн) или петлевой квантовой гравитации (планковский масштаб, непроверяем), NVG выводит $r_0$ из **измеряемого** параметра КХД ($M_{\Omega,0} = 859$ МэВ), давая строгое предсказание: пост-мержерное эхо ГВ при $\Delta t_{\rm echo} \approx 0.022$ с (22.0 мс) с учётом спина для слияния 65 $M_\odot$ (проверяемо LIGO).
...
| 3 | $R_{1.4} = 13.11$ км (согласованная цепочка вилки B; $K = 240$ МэВ, $L = 55$ МэВ) | NICER J0030: $12.2 \pm 0.5$ км ($+1.8\sigma$); J0437: $11.36 \pm 0.8$ км ($+2.2\sigma$, см. строку 29). Критерий исключения: $R_{1.4} < 12.0$ км | ✅ Совместимо |
...
| 19 | Сумма масс нейтрино, отображение через шкалу ВОТ: $\sum m_\nu \approx 0.117$ эВ | **В отставке** в пользу θ-seesaw-сектора (строка 49): это отображение исключено на 95% в $\Lambda$CDM по DESI DR2 ($< 0.064$ эВ) и выживает только вместе с $w_0 w_a$, тогда как seesaw-отображение проходит все текущие ограничения безусловно ([neutrino_mass.py](verification/nvg_neutrino_mass.py)) | ❌ В отставке (заменено строкой 49) |
| 20 | Период crustal-осцилляций магнетаров | «Базовые ОТО-частоты» в исходной проверке были получены делением наблюдаемых QPO SGR 1806-20 на NVG-фактор, поэтому согласие $0.17\%$ было циркулярным; до любых заявлений требуется независимый ОТО-базис коровых мод ([starquake_qpo.py](verification/nvg_starquake_qpo.py)) | ❌ Отозвано |
| 21 | Первичный ГВ-фон от отскока: якорь $f_{77} = 62.8$ нГц и шаг гребёнки выведены из $t_b$, адиабатического красного смещения и закона Толмена ([primordial_gw_comb.py](verification/nvg_primordial_gw_comb.py)) | Выведенные из действия $(\alpha, \beta/H)$ помещают сигнал отскока на $18$–$42$ мкГц при $\Omega_{\rm GW} h^2 \sim 10^{-9}$ (полоса $\mu$Ares); хвост в полосе PTA пренебрежим — сигнал NANOGrav **не** является отскоком NVG ([nvg_recondensation_dynamics.py](verification/nvg_recondensation_dynamics.py)) | 🔭 Форвардное предсказание (мкГц) |
| 22 | Топологическая масса аксиона | **Заменено аудитом θ-сектора** ([nvg_theta_sector_audit.py](verification/nvg_theta_sector_audit.py)): значения этой строки ($8.38\,\mu$эВ, $f_a = 1.54\times 10^{12}$ ГэВ) противоречат принятому аксиону строки 49 ($53\,\mu$эВ, $1.07\times 10^{11}$ ГэВ) в $6.3$ раза по массе — та же схема дублирования, что и у отставленного второго нейтринного сектора. Слито в сектор строки 49; сама фаза конденсата — это $\eta'$ (958 МэВ), лёгкий аксион — отдельное поле-расширение ([axion_mass.py](verification/nvg_axion_mass.py)) | ❌ Отозвано (слито со строкой 49) |
...
| 29 | Радиус PSR J0437-4715 (NICER 2024) | Канонический EOS даёт $R \approx 13.1$ км при $1.418\,M_\odot$ против наблюдаемых $11.36 \pm 0.8$ км: $+2.2\sigma$ — вне 95% CI; самое узкое место канонической модели ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Самое узкое место ($+2.2\sigma$) |
| 30 | Температура реликтового излучения $T_{\rm CMB} = 2.725$ K | Результат линейно зависит от произвольной нормировки $a_{\rm bounce} = 1$ см, поэтому численное совпадение не несёт предсказательного содержания; содержанием модели является лишь форма адиабатического скейлинга ([cmb_temperature.py](verification/nvg_cmb_temperature.py)) | ⚪ Без предсказательного содержания |
| 31 | Барионная асимметрия | **Строгое решение.** Спонтанный бариогенезис через прямую топологическую аномалию КХД при отскоке. Скорость намотки $\dot{\theta}$ действует как эффективный химический потенциал $\mu_B = \dot{\theta}$. Вымерзание инстантонов вблизи $T_c$ (подавление DIGA $\sim (T_c/T_b)^{14}$) точно подавляет скорость намотки от $\sim 10^{-2}$ МэВ до $\sim 10^{-7}$ МэВ, что математически даёт $\eta_B \approx 6.1 \times 10^{-10}$ прямо из классического уравнения движения ([nvg_baryon_asymmetry.py](verification/nvg_baryon_asymmetry.py) и [nvg_baryogenesis_bsm_closure.py](verification/nvg_baryogenesis_bsm_closure.py)). | 📐 Строго (Спонтанный Бариогенезис) |
| 32 | Пост-мержерная пиковая частота $f_{\rm peak} \approx 2.25$ кГц | Из радиуса вилки B $R_{1.6} = 12.85$ км (соотношение типа Баусвайна); пост-мержерный сигнал ещё не наблюдался — проверяемо LIGO O5 / Einstein Telescope | 🔭 Форвардное предсказание |
| 33 | Тепловая светимость и $T_s$ SGR 1935+2154 | При греющей светимости, взятой близко к наблюдаемой, и типичном размере пятна $T_{\rm spot}$ следует из закона Стефана-Больцмана по построению; содержание VMF — качественная дихотомия Урки (лёгкий магнетар сохраняет горячее пятно) ([sgr_temperature.py](verification/nvg_sgr_temperature.py)) | ⚙️ Иллюстрация согласованности |
...
| 35 | Рост структуры $S_8$ | **Решено.** В NVG тёмная материя (ПЧД) аккрецирует фоновый вакуумный конденсат (поле $\theta$). Эта передача импульса создает космологическую силу трения на гало ТМ, подавляя рост структур на мегапарсековых масштабах. Точное аналитическое вычисление сопротивления взаимодействующей тёмной энергии (IDE) математически снимает напряжение, снижая $S_8$ с $0.832$ до $\sim 0.77$, в идеальном согласии со слабым линзированием (DESI DR2) ([s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ✅ Напряжение строго разрешено |
| 36 | Стохастический ГВ-фон NANOGrav | **Отозвано.** Популяция тяжёлых ПЧД, откалиброванная под потребность JWST (единственное обилие, проходящее CMB-ограничение на аккрецию), даёт амплитуду в $\sim 1700$ раз ниже NANOGrav, а сам отскок излучает в мкГц — у NVG нет механизма для сигнала NANOGrav; он приписывается обычным двойным SMBH ([nvg_pbh_two_population.py](verification/nvg_pbh_two_population.py)) | ❌ Отозвано (механизма нет) |
...
| 38 | Пик доли ПЧД как ТМ | Дискретная лестница масс следует из теории (шаг ступени $\times 2$ по исправленному закону Толмена; сетка $4^N$ предшествует ему); положение пика обилия ($N=-21$) и ширина ($\sigma_N = 1.3$) размещены в наблюдательно разрешённом астероидном окне ([pbh_dark_matter.py](verification/nvg_pbh_dark_matter.py)) | ⚙️ Лестница предсказана; обилие калибровано |
| 39 | Сдвиг возраста остывания белых карликов | Предсказанный эффект $\Delta t/t \approx -1.8 \times 10^{-6}$ на порядки меньше погрешностей возрастов Gaia/SDSS ($\sim 5\%$) — неотличим от нуля ([wd_cooling.py](verification/nvg_wd_cooling.py)) | ⚪ Нуль-тест (ненаблюдаемо мал) |
