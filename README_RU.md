...
| 29 | Радиус PSR J0437-4715 (NICER 2024) | Канонический EOS даёт $R = 12.55$ км при $1.418\,M_\odot$ против наблюдаемых $11.36 \pm 0.8$ км: $+1.5\sigma$ — вне 68% CI, внутри 95%; самое узкое место канонической модели ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Самое узкое место ($+1.5\sigma$) |
...
| 47 | Принцип неопределённости — теорема, не постулат | $\Delta x \cdot \Delta p \geq \hbar/2$ = неравенство Коши-Буняковского для $\nabla\log\mathcal{W}$ и $\nabla\theta$ вакуумного конденсата. Неравенство — строгая математика; $\hbar$ входит через отождествление $p = \hbar\nabla\theta$ — постулат перемещён, а не устранён ([heisenberg_proof.py](verification/nvg_heisenberg_proof.py)) | 📐 Теорема (при $p = \hbar\nabla\theta$) |
...
| 50 | Квантовая гравитация без квантования | $T_H = \hbar c^3/(8\pi G M k_B) = \hbar/(k_B \tau_\theta)$ — температура Хокинга = термализация θ-поля на горизонте (точное совпадение). $S_{BH} = A/(4l_{Pl}^2) = N_\theta$ — энтропия = подсчёт θ-мод. Отскок при $\rho_c/\rho_{Pl} \sim 10^{-77}$ — полностью полуклассически ([quantum_gravity.py](verification/nvg_quantum_gravity.py)) | ❓ Гипотеза (эвристическое отождествление; Limitations препринта) |
...
- **Приливные деформации GW170817:** Канонический EOS даёт бинарное $\tilde{\Lambda} \approx 610$ — внутри 90% CI LIGO ($[70, 720]$, $+0.8\sigma$ от центра); событие с $\tilde{\Lambda} < 400$ исключит модель.
...
- **Радиус PSR J0437-4715 (NICER):** Канонический радиус $R = 12.55$ км на $+1.5\sigma$ выше измерения NICER 2024 ($11.36 \pm 0.8$ км при $1.418 M_\odot$) — внутри 95%, но это самое узкое место модели; подтверждённый $R < 12.0$ км создаст напряжение для канонического EOS (`verification/nvg_nicer_j0437_check.py`).
...
14. **Гравитационное красное смещение и f_peak:** Кривые для популяции НЗ по каноническому EOS: $z_{surf} \approx 0.221$ для $1.4 M_\odot$ (цель для STROBE-X/eXTP) и пиковая частота post-merger $f_{peak} \approx 2.42$ кГц для LIGO O5.
