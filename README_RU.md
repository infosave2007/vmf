...
| 54 | Критическая масса горизонта | $M_{\rm crit} \in [0.97, 1.01]\,M_\odot$. Выведено из точного измерения lattice QCD $M_{\Omega,0} = 859 \pm 8$ МэВ ($M_{\rm crit} \propto M_{\Omega,0}^{-2}$). Объекты легче этого порога (например, первичные ЧД в астероидном окне) не могут сформировать горизонт событий, оставаясь "голыми" регулярными де-Ситтеровскими остатками. Примечание: это фундаментальный гравитационный предел, полностью независимый от астрофизического звёздного "mass gap" ($2-5\,M_\odot$). | 🔭 Прямое предсказание |
