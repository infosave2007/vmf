...
Модель заменяет сингулярность чёрной дыры **регулярным ядром де Ситтера** (метрика Хейворда), где масштаб ядра $r_0 = (3M / 4\pi\rho_c)^{1/3}$ фиксирован якорем КХД — не является свободным параметром. Для ЧД массой $10\,M_\odot$ модель даёт два горизонта (внутренний Коши: 1.15 км, внешний: 29.34 км) и конечный скаляр Кречмана $K(0) = 14.86\,\text{км}^{-4}$.

**Механизм разрешения сингулярности:** При плотности $\rho \to \rho_c$ нарушается условие сильной энергии ($\varepsilon + 3P < 0$), останавливая коллапс и заменяя сингулярность вакуумом де Ситтера. Это тот же механизм, что производит космологический отскок, применённый на звёздных масштабах. Топологически это ядро де Ситтера действует как **фиксированная точка потока Риччи (солитон эйнштейновского многообразия)**. Как и в доказательстве гипотезы Пуанкаре Перельманом, где такие точки максимизируют приведенный объем (reduced volume) и останавливают дальнейший коллапс ($\kappa$-non-collapsing theorem), сходимость ядра NVG к чистой геометрии де Ситтера при $\rho_c$ дает строгую геометрическую гарантию стабильности, заменяя стандартный аргумент о нарушении энергоусловий структурным топологическим пределом.
