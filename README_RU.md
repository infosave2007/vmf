...
| 54 | Критическая масса горизонта | $M_{\rm crit} \in [0.97, 1.01]\,M_\odot$. Выведено из точного измерения lattice QCD $M_{\Omega,0} = 859 \pm 8$ МэВ ($M_{\rm crit} \propto M_{\Omega,0}^{-2}$). В замкнутой форме $M_{\rm crit} = \tfrac{9}{8\sqrt{2\pi}}\,M_{\rm Pl}^3/M_{\Omega,0}^2$ — комбинация чандрасекаровского типа $M_{\rm Pl}^3/m^2$ при массе КХД-конденсата, поэтому близость к $1\,M_\odot$ — *размерная* (та же причина, по которой пределы белых карликов / нейтронных звёзд лежат около $M_\odot$), а не подгонка ([nvg_mcrit_chandrasekhar.py](verification/nvg_mcrit_chandrasekhar.py)). Объекты легче этого порога (например, первичные ЧД в астероидном окне) не могут сформировать горизонт событий, оставаясь "голыми" регулярными де-Ситтеровскими остатками. Примечание: это фундаментальный гравитационный предел, полностью независимый от астрофизического звёздного "mass gap" ($2-5\,M_\odot$). | 🔭 Прямое предсказание |
