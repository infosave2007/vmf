...
### 1. Уникальные подтверждённые предсказания (Direct Discoveries)
...
| 6 | Происхождение Магнетаров | Объясняет $10^{15}$ Гс (усиление в 7.4 раза из-за падения $\epsilon_{eff}$) | ✅ Решает парадокс динамо-модели |
| 7 | Непрерывный спектр масс ПЧД ($4^N$) | Сшивает Тёмную Материю ($10^{-14} M_\odot$) и ранние квазары JWST ($10^6 M_\odot$) | ✅ Концептуально доказано |
| 8 | Joint Multi-Messenger Inference | Приведенный $\chi^2_\nu = 0.63$ для NICER+LIGO+Остывание без подгонки параметров | ✅ Глобальная согласованность |

### 2. Строгие проверки на совместимость (Null Tests & Consistency Checks)
...
| Приливная деформируемость | $\Lambda_{1.4} \approx 470$ | GW170817: $\Lambda_{1.4} = 190^{+390}_{-120}$ (в интервале) |
| Темная Энергия (DESI) | Циклическая эволюция: $w_0 = -0.83$, $w_a = -1.05$ | DESI DR1: совместимо в пределах 1.3σ |
...
### 3. Ожидают проверки (Falsifiable Forecasts & Risk Claims)
...
| **Сдвиг масс мезонов** | $M_\rho^* \approx 595$ МэВ при $2n_0$ (уменьшение на 23.2%) | Направлен запрос в коллаборацию HADES / CBM (FAIR) |
| **Гравитационное эхо** | Специфические задержки $\Delta t \approx 0.0445$ с | Целевой поиск в архивах LIGO O4/O5 (Требует вычислительных мощностей) |
| **Красный сдвиг НЗ** | $z_{\rm surf}(1.4 M_\odot) \approx 0.286$ | STROBE-X / eXTP (будущие рентгеновские обсерватории) |
| **Пост-мержер $f_{\rm peak}$** | $f_{\rm peak} \approx 3284$ Гц (по EOB суррогату) | LIGO O5 / Einstein Telescope (будущие детекторы) |
