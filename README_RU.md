...
| 2 | $M_{\max} = 2.05\,M_\odot$ (вычислено; прежние 2.25 были артефактом края таблицы EOS — см. [nvg_ns_parameter_scan.py](verification/nvg_ns_parameter_scan.py)) | PSR J0740+6620: $2.08 \pm 0.07\,M_\odot$ ($-0.4\sigma$); любая подтверждённая НЗ тяжелее $\sim 2.2\,M_\odot$ исключает модель | ⚠️ Совместимо, фальсифицируемо на краю |
| 3 | $R_{1.4} = 12.55$ км (вычислено из канонического EOS; прежние 12.0 были прописаны вручную) | NICER J0030: $12.2 \pm 0.5$ км ($+0.7\sigma$); J0437: $11.4 \pm 1.0$ км ($+1.2\sigma$); $R_{1.4} < 12.0$ км исключает модель | ⚠️ Совместимо, фальсифицируемо на краю |
| 4 | Инстантон Генезиса $r_c \to$ е-фолды ограничены интервалом $N_e \in [52.68, 53.38]$ для цикла $n=77$; конкретное $N_e = 53.08$ **калибруется** по локальному $H_0$, а не выводится | $R_{H0} = c/H_0 \approx 1.27 \times 10^{28}$ см; $N_e = \ln(R_{H0}/r_c)$ вычисляется из $H_0 = 72.8$ км/с/Мпк как входного параметра (см. [nvg_hubble_tension.py](verification/nvg_hubble_tension.py) / [nvg_genesis_observable.py](verification/nvg_genesis_observable.py)) | ⚠️ Калибровка (предсказанием является интервал) |
| 5 | Дихотомия остывания НЗ через порог прямой Урки при $1.45 M_\odot$ | Cas A (медленное) vs Vela (быстрое); ВАЖНО: положение порога **калибровано** — $\alpha_v$ в [nvg_direct_urca.py](verification/nvg_direct_urca.py) настроен на 1.45 (и отличается от канонического $\alpha_v$ EOS) | ⚠️ Калибровка |
| 6 | Тидальная деформируемость: $\Lambda_{1.4} = 519$, бинарная $\tilde\Lambda \approx 610$ (TOV + Hinderer, [nvg_tidal_deformability.py](verification/nvg_tidal_deformability.py); прежние 177 принадлежали параметризации, опровергнутой J0740) | GW170817: $\tilde\Lambda = 300\,[70, 720]$ — внутри 90% CI при $+0.8\sigma$; $\tilde\Lambda < \sim 400$ в будущем событии исключит модель | ⚠️ Совместимо, фальсифицируемо на краю |
...
| 9 | Постоянная Хаббла без подгонки: жёсткие границы $H_0 \in [54.3, 108.5]$ км/с/Мпк из горизонтов поворота цикла $n=77$ ($R_{77} = r_c \cdot 2^{76}$); средняя точка цикла (нулевая информация о фазе) даёт $H_0 = H_{77}/\sqrt{2} \approx 76.8$ км/с/Мпк (центр зависит от меры: $\approx 62$–$81$) | SH0ES ($73.04 \pm 1.04$ км/с/Мпк) в пределах 5.1% от бесподгоночной средней точки и внутри разброса мер; Planck ($67.4$) тоже внутри разброса; точное точечное совпадение по-прежнему требует калибровки внутрицикловой фазы; путь через ИК-обрезание к сдвигу планковского фита к 72.8 опровергнут количественным ре-фитом ([nvg_cmb_lowl_refit.py](verification/nvg_cmb_lowl_refit.py)) | ⚠️ Интервал + средняя точка без подгонки; натяжение не разрешено |
...
| 13 | Эффективная диэлектрическая проницаемость вакуума: $\varepsilon_{\rm eff} \approx 0.135\,\varepsilon_0$ в ядрах НЗ (из $e^{-2\alpha_v f_{\rm melt}}$ с ручными параметрами плавления $\kappa$; $\kappa_1 = 0.21$ здесь против $0.25$ в EOS — не согласовано) | Усиливало бы затравочные поля магнетаров в $1/\sqrt{\varepsilon_{\rm eff}} \approx 2.7\times$ | ⚠️ Оценка масштаба (зависит от $\kappa$) |
| 14 | Реликтовая тёмная материя: $\Omega_{\rm DM} = 0.268$ — **наблюдательный вход**: скрипт инвертирует его для вывода самодействия $\lambda_v$ | Проверяемое содержание — попадание выведенного $\lambda_v$ в диапазон скаляров $f_0(1370)$–$f_0(1500)$ ([nvg_relic_dark_matter.py](verification/nvg_relic_dark_matter.py)) | ⚠️ Калибровка (инверсия) |
| 15 | Скорость звука в ядре НЗ: $c_{s}^2 \to 1/3$ в кварковой фазе **по построению** ($c_{s,q}^2 = 1/3$ — входной параметр CSS-анзаца, а не выведенный результат) | Согласуется с совместными пределами NICER+LIGO ([speed_of_sound_curve.py](verification/nvg_speed_of_sound_curve.py)) | ⚠️ По построению |
...
| 17 | Совместный NS multi-messenger фит: редуцированный $\chi_\nu^2 \approx 0.7$ с каноническими (вычисленными) предсказаниями | Все отклонения $< 1\sigma$, **при условии** ядерно-калиброванных параметров формы EOS и выбранного сканом перехода — это не фит без параметров ([nvg_joint_ns_inference.py](verification/nvg_joint_ns_inference.py)) | ⚠️ Совместимо (условно) |
...
Все 52/56 предсказаний выводятся из единственного параметра $M_\Omega = 859$ МэВ. Оптический канал различает $\sqrt{1-x}$ от линейной модели ($\Delta r = 0.087$), подтверждая внутреннюю согласованность математической структуры.
