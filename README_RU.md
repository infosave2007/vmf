...
При полярном разложении комплексного параметра порядка:
$$ \Phi(x) = \frac{1}{\sqrt{2}} \mathcal{W}(x) e^{i \theta(x)} $$
под действием $U(1)$ ковариантной производной:
$$ D_\mu = \partial_\mu - i q A_\mu $$
кинетический член сектора скалярного поля записывается как:
...
С потенциалом Хиггса-Гинзбурга-Ландау:
$$ V(\Phi) = -\mu^2 \Phi^\dagger \Phi + \lambda (\Phi^\dagger \Phi)^2 $$
уравнения Эйлера-Лагранжа дают уравнение движения для вещественной амплитуды $\mathcal{W}(x)$:
$$ \square \mathcal{W} + \left[ \lambda \mathcal{W}^2 - (\mu^2 + g_\mu g^\mu) \right] \mathcal{W} = 0 $$
где калибровочно-инвариантный градиент равен:
$$ g_\mu \equiv \partial_\mu \theta - q A_\mu $$
В пространственно-подобных режимах, когда:
$$ g_\mu g^\mu \approx -|\vec{g}|^2 $$
эффективный потенциал амплитудного поля принимает вид:
$$ V_{\text{eff}}(\mathcal{W}) = \frac{1}{2} (|\vec{g}|^2 - \mu^2) \mathcal{W}^2 + \frac{1}{4} \lambda \mathcal{W}^4 $$
* **Симметричная фаза (Плавление):** При $|\vec{g}|^2 < \mu^2$ вакуумное ожидаемое значение (VEV) равно:
$$ \mathcal{W}_0 = \sqrt{\frac{\mu^2 - |\vec{g}|^2}{\lambda}} > 0 $$
Когда плотность энергии внешнего поля превышает порог массы ($|\vec{g}|^2 \ge \mu^2$), эффективная квадратичная масса становится положительной, переводя минимум потенциала в:
$$ \mathcal{W}_0 = 0 $$
что означает полное плавление вакуумного конденсата.
