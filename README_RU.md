...
| 2 | $M_{\max} = 2.07\,M_\odot$ (согласованная цепочка вилки B, [nvg_fork_b_full_chain.py](verification/nvg_fork_b_full_chain.py)) | PSR J0740+6620: $2.08 \pm 0.07\,M_\odot$ ($-0.1\sigma$). Критерий исключения: подтверждённая НЗ выше $\sim 2.2\,M_\odot$. Систематики: модель коры, $c_{s,q}^2 = 0.50$ | ✅ Совместимо |
| 3 | $R_{1.4} = 12.49$ км (согласованная цепочка вилки B; $K = 240$ МэВ, $L = 55$ МэВ) | NICER J0030: $12.2 \pm 0.5$ км ($+0.6\sigma$); J0437: $11.36 \pm 0.8$ км ($+1.4\sigma$, см. строку 29). Критерий исключения: $R_{1.4} < 12.0$ км | ✅ Совместимо |
...
| 6 | Приливная деформируемость: $\Lambda_{1.4} = 253$, бинарная $\tilde\Lambda \approx 313$ (согласованная цепочка вилки B + интегрирование Хиндерер) | GW170817: $\tilde\Lambda = 300\,[70, 720]$ — в центре 90% CI ($+0.05\sigma$). Критерий исключения: $\tilde\Lambda$ вне $[100, 650]$ в будущем громком событии | ✅ Совместимо (центр) |
...
| 10 | Гравитационное красное смещение поверхности: $z_{\rm surf} = 0.223$ для $1.4\,M_\odot$ НЗ (из вычисленного $R_{1.4} = 12.49$ км) | Прямые измерения отсутствуют; форвардное предсказание, проверяемое STROBE-X/eXTP ([nvg_ns_redshift.py](verification/nvg_ns_redshift.py)) | 🔭 Форвардное предсказание |
...
| 29 | Радиус PSR J0437-4715 (NICER 2024) | Канонический EOS даёт $R = 12.49$ км при $1.418\,M_\odot$ против наблюдаемых $11.36 \pm 0.8$ км: $+1.4\sigma$ — вне 68% CI, внутри 95%; самое узкое место канонической модели ([nicer_j0437_check.py](verification/nvg_nicer_j0437_check.py)) | ⚠️ Самое узкое место ($+1.4\sigma$) |
...
| 32 | Пост-мержерная пиковая частота $f_{\rm peak} \approx 2.51$ кГц | Из радиуса вилки B $R_{1.6} = 12.27$ км (соотношение типа Баусвайна); пост-мержерный сигнал ещё не наблюдался — проверяемо LIGO O5 / Einstein Telescope | 🔭 Форвардное предсказание |
