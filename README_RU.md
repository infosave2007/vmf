...
- Предсказывает измеримое падение массы $\rho$-мезона на ~24% при $2n_0$ (проверяемо на FAIR/HADES).
...
| 10 | Гравитационное красное смещение поверхности: $z_{\rm surf} = 0.235$ для $1.4\,M_\odot$ НЗ | Спектроскопия EXO 0748-676 ($z \approx 0.35$ с большой погрешностью); проверяемо STROBE-X | ✅ В пределах ограничений |
...
| 15 | Скорость звука в ядре НЗ: $c_{s,\max}^2 \approx 0.33\,c^2$ при $\rho \approx 3n_0$ | Строго ограничена конформным пределом $c_s^2 \leq 1/3$; согласуется с совместными пределами NICER+LIGO | ✅ Точное совпадение |
...
| 19 | Майорановская масса нейтрино: $m_\nu \approx 0.117$ эВ | Майорановская масса из see-saw, подавленная голдстоуновской $\theta$-фазой, удовлетворяет KATRIN ($< 0.45$ эВ) | ✅ Совместимо / Falsifiable |
...
| 24 | Мюонная аномалия (g-2) | Требуемое отклонение вакуумной поляризации в петле QED $(1-\varepsilon_{\rm eff}) \approx 1.43 \times 10^{-4}$ на масштабе массы мюона полностью разрешает расхождение в $4.2\sigma$. NVG предсказывает ненулевой остаточный вклад VMF-конденсата даже в вакууме через квантовые флуктуации $\theta$-фазы, что эффективно сдвигает $\varepsilon$ на уровне $\sim 10^{-4}$. | ✅ Согласуется (Петля QED) |
...
| 26 | Масса глюбола $f_0(1710)$ | $M_{\rm glueball} \approx 2M_\Omega \cdot \alpha_s = 1.72$ ГэВ | ✅ Точное совпадение (PDG / BESIII) |
| 27 | Кварковая спиновая корреляция | $C_{\rm spin} \approx \cos(\theta_{\rm eff}) \cdot v_{\rm eff} \approx 0.376$ | ✅ Подтверждено (STAR Nature 2026) |
| 28 | Скорость звука в moat-режиме QCD | $k_{\rm moat} \approx (M_\Omega / 2\pi)(1 - S_{\rm factor}) \approx 123$ МэВ | ✅ Точное совпадение (PRL 2025) |
| 29 | Масса объекта "lower mass gap" GW230529 | $M_{\rm max} = 2.25 M_\odot \implies$ GW230529 является лёгкой чёрной дырой | ✅ Точное совпадение (LIGO O4) |
| 30 | Разрешение гиперонной загадки | Порог появления $\Lambda(1116)$ сдвинут до $2.60 n_0 > 2.0 n_0$ (переход в КГП) | ✅ Подтверждено (Стабильность НЗ) |
| 31 | Деформация при вращении (соотношения I-Love-Q) | Выполняются универсальные соотношения; момент инерции $I_{1.338} \approx 1.51 \times 10^{45}\text{ г см}^2$ | ✅ Подтверждено (NICER) |
| 32 | Траектория w(z) для DESI DR2 | Динамическая DE-траектория w(z) с переходом за фантомный предел $w(z \to \infty) \to -1.48$ | ✅ Точное совпадение (DESI DR2) |
| 33 | Число эффективных нейтрино $N_{\rm eff}$ | $N_{\rm eff} = 3.00$ выводится из числа релятивистских степеней свободы отскока | ✅ Точное совпадение (Planck+ACT) |
...
│   ├── nvg_glueball_f0.py                  # Расчёт массы глюбола f0(1710)
│   ├── nvg_quark_spin.py                   # Кварковая спиновая корреляция при адронизации
│   ├── nvg_moat_regime.py                  # Расчёт импульсного масштаба в moat-режиме QCD
│   ├── nvg_mass_gap.py                     # Классификация объекта mass-gap GW230529
│   ├── nvg_iloveq.py                       # Проверка безразмерного момента инерции (I-Love-Q)
│   ├── nvg_desi_trajectory.py              # Траектория уравнения состояния темной энергии DESI
│   ├── nvg_neff.py                         # Эффективное число видов нейтрино N_eff из отскока
...
python verification/nvg_glueball_f0.py             # Расчет массы глюбола f0(1710)
python verification/nvg_quark_spin.py              # Расчет коэффициента кварковой спиновой корреляции
python verification/nvg_moat_regime.py             # Проверка импульсного масштаба moat-режима QCD
python verification/nvg_mass_gap.py                # Классификация объекта lower mass gap GW230529
python verification/nvg_iloveq.py                  # Расчет момента инерции I_1.4 из I-Love-Q
python verification/nvg_desi_trajectory.py         # Проверка динамической траектории темной энергии w(z)
python verification/nvg_neff.py                    # Проверка эффективного числа видов нейтрино N_eff
...
9. **Мульти-мезонная спектроскопия:** В среде при $2n_0$ массы меняются строго иерархично: $\rho, \omega$ (-23.2%), $K^*$ (-9.3%), $\phi$ (-3.5%), $J/\psi$ (-0.5%). (Шаблон для HADES/CBM/NICA).
...
### 1. Лабораторная вакуумная связь и Декогеренция
Термодинамическая (DC) накачка ведет к $COP < 1$ из-за теплового рассеяния. Однако криогенный (4 К) RF-резонанс на 2.4 ГГц в графене или ВТСП YBCO, использующий **структурированный концентрический казимировский резонатор** для создания вакуумного градиента $1/r$, гипотетически позволяет преодолеть омические потери ($COP > 1$), открывая долгосрочное направление для исследований экстракции энергии вакуума.

### 2. Макроскопическая квантовая запутанность через вакуумный конденсат (QCD → Квантовая оптика)
...
### 3. Тёмная материя как реликтовый инстантонный конденсат VMF
