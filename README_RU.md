...
| 4 | Инстантон Генезиса $r_c \to$ $N_e = 53.08$ е-фолда (топологически ограничен $[52.68, 53.38]$ циклом $n=77$) | $R_{H0} = c/H_0 \approx 1.27 \times 10^{28}$ см (согласуется с $t_0 \approx 13.8$ млрд лет) | ✅ Точное совпадение |
...
- **Темная энергия DESI DR2 $w(z)$:** Предсказанная траектория циклической модели ($w_0 = -0.888, w_a = -0.597$) лежит в пределах $1.7\sigma$ от наблюдаемых значений во всех отдельных бинах по красному смещению $z \ge 0.15$ (при этом совместная точка параметров CPL $(w_0, w_a)$ находится на уровне $2.74\sigma$ совместного эллипса контуров DESI DR2).
