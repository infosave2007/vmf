...
| 6 | Тидальная деформируемость: $\Lambda_{1.4} \approx 177$ (выведена из TOV + Hinderer $y$-интегрирования в [nvg_tidal_deformability_gw170817.py](verification/nvg_tidal_deformability_gw170817.py)) | GW170817: $\Lambda_{1.4} = 190^{+390}_{-120}$ (попадание прямо в центр posterior) | ✅ Точное совпадение |
...
| Темная Энергия (DESI) | Циклическая эволюция: $w_0 = -0.890$, $w_a = -0.574$ | Выводятся динамически: отдельные бины согласуются на $1.7\sigma$, тогда как совместный двумерный эллипс CPL ($w_0$, $w_a$) находится на уровне $2.74\sigma$ контуров DESI DR2 ([nvg_dark_energy_w0wa.py](verification/nvg_dark_energy_w0wa.py) / [nvg_dark_energy_desi.py](verification/nvg_dark_energy_desi.py)) |
...
- **Темная энергия DESI DR2 $w(z)$:** Предсказанная траектория циклической модели ($w_0 = -0.890, w_a = -0.574$) согласуется с бинированными данными DESI DR2. В частности, отдельные бины согласуются на $1.7\sigma$ (в одномерном анализе), тогда как совместный двумерный эллипс ограничений CPL ($w_0$, $w_a$) находится на уровне $2.74\sigma$ совместных контуров DESI DR2.
