...
| 19 | Сумма майорановских масс нейтрино: $\sum m_\nu \approx 0.117$ эВ (оценка масштаба; внешний параметр — шкала ВОТ) | DESI DR2 исключает это значение на 95% в $\Lambda$CDM ($< 0.064$ эВ), но допускает в космологии $w_0 w_a$ ($< 0.16$ эВ), которую NVG сама предсказывает — пара проверяется совместно ([neutrino_mass.py](verification/nvg_neutrino_mass.py)). Замечание: строка 49 даёт альтернативное отображение с $\sum m_\nu = 59$ мэВ — два сектора взаимоисключающи и требуют согласования | ⚙️ Оценка масштаба (жизнеспособна только вместе с $w_0 w_a$) |
...
| 35 | Рост структуры $S_8$ | Честный учёт: динамическая тёмная энергия NVG слегка усиливает рост ($+1.4\%$), давая $S_8 \approx 0.843$ — $4.0\sigma$ от лензинга (против $3.3\sigma$ в $\Lambda$CDM); предложенный механизм ядер де Ситтера короче требуемых $7.8\%$ на $\sim 45$ порядков ([s8_tension_check.py](verification/nvg_s8_tension_check.py)) | ❌ Открытая проблема (потенциальный фальсификатор) |
