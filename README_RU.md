...
Конечный размер инстантона $r_c$, растянутый $N_e \approx 53$ е-фолдами (калибруется по локальному $H_0$), накрывает современный хаббловский горизонт по построению; фальсифицируемым содержанием является получающаяся детерминированная инфракрасная обрезка, подавляющая мультиполи РИ $\ell=2,3$ — не космическая дисперсия, а проверяемое следствие масштаба Генезиса.
